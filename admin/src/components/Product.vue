				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						<tr v-for="(product  , index ) in products" :key="product._id">
							...
							<td>{{ product.price }}</td>
							<td>
                             <img :src="`http://localhost:3000/uploads/${product.image}`" alt="" width="30px" height="30px" />
							</td>
							<td>{{ product.quantity }}</td>
							<td>
								<a href="#editEmployeeModal" class="edit" data-toggle="modal" @click="openEditModal(product)"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
								<a href="#deleteEmployeeModal" class="delete" data-toggle="modal" @click="openDeleteModal(product._id)"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<div class="modal-content">
				<form @submit = "submitAdd" enctype="multipart/form-data">
					...
					<div class="modal-body">					
						...
						<div class="form-group">
							<label>Price</label>
							<input v-model="input_price" type="number" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Image</label>
						...
						<div class="form-group">
							...
							<input v-model="input_quantity" type="number" class="form-control" required>
						</div>					
					</div>
				...
				<form  enctype="multipart/form-data">
					...
					<div class="modal-body">					
						...
						<div class="form-group">
							...
							<div v-if="editProduct">
							  <img :src="`http://localhost:3000/uploads/${editProduct.image}`" alt="" width="50px" height="50px" />
                            </div>
						</div>
						<div class="form-group">
							<label>Quantity</label>
							<div v-if="editProduct">
								<input type="text" class="form-control" v-model="editProduct.quantity" required>
							</div>
						</div>					
					</div>
					<div class="modal-footer">
...
<script setup>
   ...
   let onSubmit = async () =>{
	    let productEdited = new FormData()
		console.log(file.value)
		productEdited.append('file' , file.value ) 
		productEdited.append('name' , editProduct.value.name )
		productEdited.append('price' , editProduct.value.price )
		productEdited.append('quantity' , editProduct.value.quantity )
	   try{
       await axios.put(`http://localhost:3000/product/${editProduct.value._id}`, productEdited,  {
        headers:{
   ...
   //delete product
   let id_product = ref(null) ;
   let openDeleteModal = (id) => {
      id_product.value = id ;
   };
   let deleteSubmit = async () =>{
      try{
         await axios.delete(`http://localhost:3000/product/${id_product.value}`);
         fetchProducts();
	 // Đóng modal thủ công
   ...
   let input_price = ref(null)
   let input_quantity = ref(null)
   let submitAdd = async () =>{
	if (!input_file.value) {
        alert("Please select an image file.");
        return;
    }
      let productAdd = new FormData()
	    console.log(input_file.value)
		productAdd.append('file' , input_file.value ) 
		productAdd.append('name' , input_name.value)
		productAdd.append('price' , input_price.value )
		productAdd.append('quantity' , input_quantity.value )
		try{
		let response =  await axios.post('http://localhost:3000/product/store', productAdd,  {
        headers:{
        'Content-Type': 'multipart/form-data',
        },
    })
