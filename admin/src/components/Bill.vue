			...
			<div class="table-wrapper">
				<div class="table-title">
					<div class="row">
						...
						<!-- <div class="col-xs-6">
							<a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Add New Bills</span></a>
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<tr v-for="(bill  , index ) in Bills" :key="bill._id">
							<td>{{ bill.customer_id.name }}</td>
                            <td>{{ bill.customer_id.email }}</td>
                            <td> {{ bill.customer_id.address }}</td>
							 <td>{{ calculateTotalPrice(bill.items) }}</td>
							<td>
								<select v-model="bill.status" class="btn btn-primary" @change="updateStatus(bill)">
							...
							<td>
								<a href="#editEmployeeModal" class="edit" data-toggle="modal" @click="openViewDetail(bill)"><i class="bi bi-eye-fill"  data-toggle="tooltip" title="Edit"></i></a>
								<a href="#deleteEmployeeModal" class="delete" data-toggle="modal" @click="openDeleteModal(bill._id)"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
							</td>
						</tr>
...
<script setup>
   ...
   function calculateTotalPrice(items) {
            if (!items || !items.length) return 0; // Handle case if items are undefined or empty
            return items.reduce((total, item) => {
                return total + (item.price * item.quantity);
            }, 0);
        }
    //update status
   let updateStatus = async(bill) => {
	    console.log(bill.status)
		try {
         await axios.put(`http://localhost:3000/bill/${bill._id}`, 
        { status: bill.status }, 
        {
            headers: {
   ...
   //delete customer
   let id_bill = ref(null) ;
   let openDeleteModal = (id) => {
      id_bill.value = id ;
   };
   let deleteSubmit = async () =>{
      try{
         await axios.delete(`http://localhost:3000/bill/${id_bill.value}`);
         fetchBills();
      } catch (error){
        console.log(error);
      }
